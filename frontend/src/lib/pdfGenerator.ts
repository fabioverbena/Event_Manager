...
const GRENKE_NOTE = '*Alle rate va aggiunta IVA. La prima rata si paga 5/6 mesi dopo la consegna. Bene strumentale totalmente deducibile. Prezzo comprensivo di trasporto e montaggio. È previsto il riscatto a termine del noleggio.'

const GRENKE_MODELLO_TABELLE_STANDARD: Partial<Record<string, GrenkeLeasingTable>> = {
  leo2: {
    rows: [
...
const GRENKE_MODELLO_TABELLE_PERSONALIZZATO: Partial<Record<string, GrenkeLeasingTable>> = {
  leo2: {
    rows: [
...
const renderGrenkeLeasingCondizioni = (doc: jsPDF, modelKey: string, startY: number, colore: 'standard' | 'personalizzato' = 'standard') => {
...
const renderDocumentoPage = async (doc: jsPDF, ordine: Ordine, tipoDocumento: TipoDocumento) => {
  ...
  const grenkeColore: 'standard' | 'personalizzato' = tipoVenditaEspositoriNorm.includes('personalizzato') ? 'personalizzato' : 'standard'
  ...
  if (ordine.ha_espositori && ordine.tipo_vendita_espositori) {
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(
      `ESPOSITORI - ${isLeasingGrenke ? `LEASING GRENKE - ${grenkeColore === 'personalizzato' ? 'COLORE PERSONALIZZATO' : 'COLORI STANDARD'}` : 'VENDITA DIRETTA'}`,
      105,
      yPos,
      { align: 'center' }
    )
  ...
  if (isGrenkeDocument) {
    ...
    const rendered = renderGrenkeLeasingCondizioni(doc, modelKey, postTableY, grenkeColore)
