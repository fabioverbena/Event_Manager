...
export default function ModuloVuotoModal({ onClose, eventoCorrente }: ModuloVuotoModalProps) {
  ...
  const [grenkeColore, setGrenkeColore] = useState<'standard' | 'personalizzato'>('standard')
    ...
    try {
      await generateModuloVuoto(
        ...
        tipoSelezionato === 'Espositori'
          ? { grenkeLeasing, grenkeModel, grenkeColore }
          : undefined
      )
      onClose()
    } catch (error) {
            ...
            <div className="card bg-gray-50">
              ...
              {grenkeLeasing && (
                <div className="mt-4 space-y-3">
                  ...
                  <div>
                    <label className="label">Tipo colore</label>
                    <div className="flex gap-4 mt-1">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          checked={grenkeColore === 'standard'}
                          onChange={() => setGrenkeColore('standard')}
                          className="w-4 h-4 text-primary-600"
                        />
                        <span className="text-sm font-medium text-gray-900">Colori Standard</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="radio"
                          checked={grenkeColore === 'personalizzato'}
                          onChange={() => setGrenkeColore('personalizzato')}
                          className="w-4 h-4 text-primary-600"
                        />
                        <span className="text-sm font-medium text-gray-900">Colore Personalizzato</span>
                      </label>
                    </div>
                  </div>
